...
div#viewComments {
	...
	text-align: left;
	max-height: 300px;
	overflow: auto; /* border: solid 1px red; */
}
...
/* 		// 보내야할 데이터를 선정하는 첫번째 방법
	...
	function golikeAdd(pnum) { 
		...
		function godisLikeAdd(pnum) {
			...
			$.ajax({ url:"/MyMVC/shop/dislikeAdd.up",
				type:"POST", 
				data:{"pnum":pnum, "userid":"${sessionScope.loginuser.userid}"}, 
				dataType:"JSON", 
				success:function(json) { 
			...
			function goLikeDislikeCount() {
				...
				$.ajax({ url:"/MyMVC/shop/likeDislikeCount.up", 
					// type:"GET", 
					data:{"pnum":"${pvo.pnum}"}, 
					dataType:"JSON", 
					success:function(json) {
...
<div style="width: 95%;">
	<div class="row">
		<div class="col-md-12 line">
			<span style="font-size: 15pt; font-weight: bold;">::: 제품 상세 정보
				:::</span>
		</div>
	</div>
	<div class="row">
		<div class="col-md-3 line">
			<img src="/MyMVC/images/${pvo.pimage1}"
				style="width: 200px; height: 170px;" />
		</div>
		<div class="col-md-9 line" align="left">
			<ul style="list-style-type: none;">
				<li><span
					style="color: red; font-size: 12pt; font-weight: bold;">${pvo.spvo.sname}</span></li>
				<li>제품번호: ${pvo.pnum}</li>
				<li>제품이름: ${pvo.pname}</li>
				<li>제조회사: ${pvo.pcompany}</li>
				<li>제품정가: <span style="text-decoration: line-through;"><fmt:formatNumber
							value="${pvo.price}" pattern="###,###" />원</span></li>
				<li>제품판매가: <span style="color: blue; font-weight: bold;"><fmt:formatNumber
							value="${pvo.saleprice}" pattern="###,###" />원</span></li>
				<li>할인율: <span style="color: maroon; font-weight: bold;">[${pvo.discountPercent}%
						할인]</span></li>
				<li>포인트: <span style="color: green; font-weight: bold;">${pvo.point}
						POINT</span></li>
				<li>잔고갯수: <span style="color: maroon; font-weight: bold;">${pvo.pqty}
						개</span></li>
			</ul>
			<%-- ==== 장바구니 담기 및 바로주문하기 폼 ==== --%>
			<form name="cartOrderFrm">
				<ul style="list-style-type: none; margin-top: 50px;">
					<li style="margin-bottom: 40px;"><label for="spinner">주문개수:</label>
						<input id="spinner" name="oqty" value="1"
						style="width: 30px; height: 20px;"></li>
					<li>
						<button type="button" class="btn btn-info"
							onClick="goCart('${pvo.pnum}');" style="margin-right: 10px;">장바구니담기</button>
						<button type="button" class="btn btn-warning"
							onClick="goOrder('${pvo.pnum}');">바로주문하기</button>
					</li>
				</ul>
				<input type="hidden" name="pnum" value="${pvo.pnum}" /> <input
					type="hidden" name="saleprice" value="${pvo.saleprice}" /> <input
					type="hidden" name="sumtotalprice" /> <input type="hidden"
					name="sumtotalpoint" /> <input type="hidden" name="goBackURL"
					value="${goBackURL}" size="200" />
			</form>
		</div>
	</div>
	<div class="row">
		<div class="col-md-12 line">
			<img src="/MyMVC/images/${pvo.pimage2}"
	...
	<div class="row">
		<c:if test="${not empty imgList}">
			<c:forEach var="imgfilename" items="${imgList}">
				<div class="col-md-3 line">
					<img src="/MyMVC/images/${imgfilename}"
						style="width: 200px; height: 170px;" />
				</div>
			</c:forEach>
		</c:if>
	...
	<div class="row">
		<div class="col-md-12 line">
			<span
				style="color: white; font-weight: bold; font-size: 14pt; background-color: navy;">제품설명</span>
			<p>${pvo.pcontent}</p>
		</div>
	</div>
	<div class="row" style="margin-bottom: 50px;">
		<div class="col-md-2 col-md-offset-2">
			<img src="<%=request.getContextPath()%>/images/like.png"
				style="cursor: pointer;" onClick="golikeAdd('${pvo.pnum}');" />
		</div>
		<div class="col-md-1" id="likeCnt" align="left"
			style="color: blue; font-size: 16pt;"></div>
		<div class="col-md-2 col-md-offset-2">
			<img src="<%=request.getContextPath()%>/images/dislike.png"
				style="cursor: pointer;" onClick="godisLikeAdd('${pvo.pnum}');" />
		</div>
		<div class="col-md-1" id="dislikeCnt" align="left"
			style="color: red; font-size: 16pt;"></div>
	</div>
	...
	<form name="commentFrm">
		<div>
			<textarea cols="100" class="customHeight" name="commentContents"
