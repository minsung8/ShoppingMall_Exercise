...
public class ProductDAO implements InterProductDAO{
	...
	public int productInsert(ProductVO pvo) throws SQLException {
		...
		try {
			conn = ds.getConnection(); 
			String sql = " insert into tbl_product(pnum, pname, fk_cnum, pcompany, pimage1, pimage2, pqty, price, saleprice, fk_snum, pcontent, point) " + " values(?,?,?,?,?,?,?,?,?,?,?,?)"; 
			pstmt = conn.prepareStatement(sql); 
	...
	// tbl_product_imagefile 테이블에 추가이미지 파일명 insert 해주기 
		...
		public ProductVO selectOneProductByPnum(String pnum) throws SQLException {
			...
			try { 
				conn = ds.getConnection(); 
				String sql = " select S.sname, pnum, pname, pcompany, price, saleprice, point, pqty, pcontent, pimage1, pimage2 "+ " from "+ " ( "+ " select fk_snum, pnum, pname, pcompany, price, saleprice, point, pqty, pcontent, pimage1, pimage2 "+ " from tbl_product "+ " where pnum = ? "+ " ) P JOIN tbl_spec S "+ " ON P.fk_snum = S.snum "; 
				pstmt = conn.prepareStatement(sql); 
		...
		@Override public List<String> getImagesByPnum(String pnum) throws SQLException {
			...
			pstmt = conn.prepareStatement(sql); 
			pstmt.setString(1, pnum); rs = pstmt.executeQuery(); 
			
			while(rs.next()) { String imgfilename = rs.getString(1); // 이미지파일명 
			imgList.add(imgfilename); } } finally { close(); } return imgList; }
